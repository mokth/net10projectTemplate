...
public partial class PoCdnList : PageBase, IDisposable
{
    ...
    protected List<PoCdnListRow> CompactRows { get; set; } = [];
    ...
    protected PoCdnGridDataSource DataSource { get; private set; } = default!;

    // ── Type-derived properties ──────────────────────────────────────────────

    private bool IsDebitNote => string.Equals(_type, "DN", StringComparison.OrdinalIgnoreCase);

    protected string DocumentTypeName => IsDebitNote ? "Debit Notes" : "Credit Notes";

    protected string PageTitle => IsDebitNote ? "Purchase Debit Note" : "Purchase Credit Note";

    protected string HeroIcon => IsDebitNote ? "fa-solid fa-file-plus" : "fa-solid fa-file-minus";

    protected string MenuCode => IsDebitNote ? MenuCodes.PurchaseDebitNote : MenuCodes.PurchaseCreditNote;

    protected string GridKey => IsDebitNote ? "po-dn-list" : "po-cn-list";

    protected string NewRoute => IsDebitNote ? "/purchase/debit-notes/new" : "/purchase/credit-notes/new";

    protected string ViewRoute(string docNo) =>
        IsDebitNote ? $"/purchase/debit-notes/view/{docNo}" : $"/purchase/credit-notes/view/{docNo}";

    protected string EditRoute(string docNo) =>
        IsDebitNote ? $"/purchase/debit-notes/edit/{docNo}" : $"/purchase/credit-notes/edit/{docNo}";

    protected string SearchPlaceholder =>
        IsDebitNote ? "Search DN no, vendor…" : "Search CN no, vendor…";

    protected string FilterTitle => IsDebitNote ? "Filter debit notes" : "Filter credit notes";
    ...
    protected void OnSelectionsEvent(List<PoCdnListRow> list)
    {
        _selectedRows.Clear();
        _selectedRows.AddRange(list);
    }

    protected async Task OnButtonClick(SelectedButtonInfo<PoCdnListRow> info)
    {
    ...
    protected Task OnActionClick(SelectedButtonInfo<PoCdnListRow> info)
    {
        ...
        else if (mode == "EDIT")
        {
            ...
            if (!string.Equals(info.SelectedRow.Status, PoCdnStatuses.New, StringComparison.OrdinalIgnoreCase))
            {
    ...
    protected async Task ConfirmActionAsync()
    {
        ...
        try
        {
            var keyed = _selectedRows
                .Select(x => new PoCdnKeyedRequest { DocNo = x.DocNo, RowVersion = x.RowVersion })
                .ToList();

            PoCdnOperationResult result = ConfirmAction switch
            {
            ...
            {
                // C15: a suspended/inactive vendor does not block POST, but the operator must be told.
                var warnings = result.Posting
                    .Where(x => x.VendorWarning && !string.IsNullOrWhiteSpace(x.VendorWarningMessage))
                    .Select(x => x.VendorWarningMessage!)
                    .Distinct()
                    .ToList();
                ...
                if (warnings.Count > 0)
                {
                    summary = $"{summary} — {string.Join(" ", warnings)}";
                }
    ...
    protected static string StatusChipClass(string? status) =>
        string.Equals(status, PoCdnStatuses.Posted, StringComparison.OrdinalIgnoreCase) ? "is-on" : "is-hold";
    ...
    private Task BeginDeleteAsync()
    {
        ...
        var notNew = _selectedRows
            .Where(x => !string.Equals(x.Status, PoCdnStatuses.New, StringComparison.OrdinalIgnoreCase))
    ...
    private Task BeginPostAsync()
    {
        ...
        // C1: POSTED is operational finalization, not immutability — say so before the user commits.
        ConfirmAction = "POST";
        ConfirmMessage = $"Post {_selectedRows.Count} selected document(s)? "
            + "Posting finalizes the document operationally; it does not lock the financial correction.";
        ConfirmVisible = true;
        return Task.CompletedTask;
    }
    ...
    private Task BeginRollbackAsync()
    {
        ...
        var notPosted = _selectedRows
            .Where(x => !string.Equals(x.Status, PoCdnStatuses.Posted, StringComparison.OrdinalIgnoreCase))
            .Select(x => x.DocNo).ToList();
    ...
    {
        DataSource.UpdateFilters(new PoCdnListQuery
        {
            Type = _type,
    ...
    private async Task<(IReadOnlyList<PoCdnListRow> Rows, int TotalCount)> SearchPageAsync(
...
public sealed class PoCdnGridDataSource : GridCustomDataSource
{
    private readonly Func<PoCdnListQuery, CancellationToken, Task<(IReadOnlyList<PoCdnListRow> Rows, int TotalCount)>> _loader;
    private PoCdnListQuery _filters = new();

    public PoCdnGridDataSource(
        Func<PoCdnListQuery, CancellationToken, Task<(IReadOnlyList<PoCdnListRow> Rows, int TotalCount)>> loader)
    {
        _loader = loader;
    }

    public PoCdnListQuery CurrentQuery => Clone(_filters);
    public void UpdateFilters(PoCdnListQuery query) => _filters = Clone(query);
    ...
    private static PoCdnListQuery Clone(PoCdnListQuery source) =>
        new()
        {
            Type = source.Type,
