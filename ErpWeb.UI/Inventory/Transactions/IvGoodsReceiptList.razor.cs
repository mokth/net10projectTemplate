...
public partial class IvGoodsReceiptList : PageBase, IDisposable
{
    ...
    private readonly List<IvGoodsReceiptListRow> _selectedRows = [];
    ...
    protected List<IvGoodsReceiptListRow> CompactRows { get; set; } = [];
    ...
    protected IvGoodsReceiptGridDataSource DataSource { get; private set; } = default!;
    ...
    {
        DataSource = new IvGoodsReceiptGridDataSource(SearchPageAsync);

        CanAdd = await AccessRights.CanAsync(MenuCodes.InventoryGoodsReceipt, PermissionCodes.Add);
        CanEdit = await AccessRights.CanAsync(MenuCodes.InventoryGoodsReceipt, PermissionCodes.Edit);
        CanDelete = await AccessRights.CanAsync(MenuCodes.InventoryGoodsReceipt, PermissionCodes.Delete);
        CanPost = await AccessRights.CanAsync(MenuCodes.InventoryGoodsReceipt, PermissionCodes.Post);
        CanRollback = await AccessRights.CanAsync(MenuCodes.InventoryGoodsReceipt, PermissionCodes.Rollback);
        ...
        ActionButtons =
        [
            new() { Text = "VIEW", IConClass = "fa-regular fa-eye", Style = "primary", ToolTip = "View receipt" },
            new() { Text = "EDIT", IConClass = "far fa-edit", Style = "primary", ToolTip = "Edit receipt", Enabled = CanEdit }
        ];
    ...
    protected void OnSelectionsEvent(List<IvGoodsReceiptListRow> list)
    {
        _selectedRows.Clear();
        _selectedRows.AddRange(list);
    }

    protected async Task OnButtonClick(SelectedButtonInfo<IvGoodsReceiptListRow> info)
    {
        ...
        switch (mode)
        {
            case "NEW":
                ...
                Navigation.NavigateTo("/inventory/goods-receipts/new");
                break;
            case "DELETE":
    ...
    protected Task OnActionClick(SelectedButtonInfo<IvGoodsReceiptListRow> info)
    {
        ...
        switch (mode)
        {
            ...
            case "EDIT":
                ...
                {
                    StatusMessage = "Only NEW receipts can be edited.";
                    NavigateView(info.SelectedRow.BatchNo);
                    break;
                }

                Navigation.NavigateTo($"/inventory/goods-receipts/edit/{info.SelectedRow.BatchNo}");
                break;
        }
    ...
    protected void NavigateView(int batchNo) =>
        Navigation.NavigateTo($"/inventory/goods-receipts/view/{batchNo}");
    ...
    protected async Task ConfirmActionAsync()
    {
        ...
        try
        {
            ...
            IvGoodsReceiptOperationResult result = ConfirmAction switch
            {
                "POST" => await GoodsReceipt.PostAsync(batchNos),
                "ROLLBACK" => await GoodsReceipt.RollbackAsync(batchNos),
                _ => await GoodsReceipt.DeleteAsync(batchNos)
            };
            ...
            if (result.Succeeded)
            {
                ...
                {
                    "POST" => $"Posted {result.SucceededCount} receipt(s).",
                    "ROLLBACK" => $"Rolled back {result.SucceededCount} receipt(s).",
                    _ => "Receipt(s) deleted."
                };
                ConfirmVisible = false;
                _selectedRows.Clear();
            ...
            {
                ErrorMessage = result.ErrorMessage ?? $"Unable to {ConfirmAction.ToLowerInvariant()} receipt(s).";
                ConfirmVisible = false;
    ...
    private Task BeginDeleteAsync()
    {
        ...
        if (notNew.Count > 0)
        {
            ErrorMessage = $"Only NEW receipts can be deleted. Non-NEW: {string.Join(", ", notNew)}";
            return Task.CompletedTask;
        }

        ConfirmAction = "DELETE";
        ConfirmMessage = $"Permanently delete {_selectedRows.Count} selected receipt(s)?";
        ConfirmVisible = true;
        return Task.CompletedTask;
    }
    ...
    private Task BeginPostAsync()
    {
        ...
        {
            ErrorMessage = $"Select at most {IvPostingLimits.MaxPostSelection} receipts to post.";
            return Task.CompletedTask;
        }
        ...
        if (notNew.Count > 0)
        {
            ErrorMessage = $"Only NEW receipts can be posted. Non-NEW: {string.Join(", ", notNew)}";
            return Task.CompletedTask;
        }

        ConfirmAction = "POST";
        ConfirmMessage = $"Post {_selectedRows.Count} selected receipt(s) to stock?";
        ConfirmVisible = true;
        return Task.CompletedTask;
    }
    ...
    private Task BeginRollbackAsync()
    {
        ...
        if (_selectedRows.Count > IvPostingLimits.MaxPostSelection)
        {
            ErrorMessage = $"Select at most {IvPostingLimits.MaxPostSelection} receipts to roll back.";
            return Task.CompletedTask;
        }
        ...
        {
            ErrorMessage = $"Only POSTED receipts can be rolled back. Non-POSTED: {string.Join(", ", notPosted)}";
            return Task.CompletedTask;
        }

        ConfirmAction = "ROLLBACK";
        ConfirmMessage = $"Roll back {_selectedRows.Count} selected receipt(s)? Stock will decrease by the posted quantities.";
        ConfirmVisible = true;
        return Task.CompletedTask;
    }
    ...
    {
        DataSource.UpdateFilters(new IvGoodsReceiptListQuery
        {
    ...
    private async Task RefreshCompactPreviewAsync()
    {
        ...
        query.Skip = 0;
        query.Take = 50;
        var result = await GoodsReceipt.SearchAsync(query);
    ...
    private async Task<(IReadOnlyList<IvGoodsReceiptListRow> Rows, int TotalCount)> SearchPageAsync(
        IvGoodsReceiptListQuery query,
        CancellationToken cancellationToken)
    {
        var result = await GoodsReceipt.SearchAsync(query, cancellationToken);
        if (!result.Succeeded || result.ListPage is null)
        {
            await InvokeAsync(() =>
            {
                ErrorMessage = result.ErrorMessage ?? "Unable to load receipts.";
                TotalCount = 0;
            });
            return ([], 0);
        }
...
/// <summary>
/// Server-side paging source for Goods Receipt via <see cref="IIvGoodsReceiptService.SearchAsync"/>.
/// </summary>
public sealed class IvGoodsReceiptGridDataSource : GridCustomDataSource
{
    private readonly Func<IvGoodsReceiptListQuery, CancellationToken, Task<(IReadOnlyList<IvGoodsReceiptListRow> Rows, int TotalCount)>> _loader;
    private IvGoodsReceiptListQuery _filters = new();

    public IvGoodsReceiptGridDataSource(
        Func<IvGoodsReceiptListQuery, CancellationToken, Task<(IReadOnlyList<IvGoodsReceiptListRow> Rows, int TotalCount)>> loader)
    {
        _loader = loader;
    }

    public IvGoodsReceiptListQuery CurrentQuery => Clone(_filters);

    public void UpdateFilters(IvGoodsReceiptListQuery query) =>
    ...
    {
        if (string.Equals(fieldName, nameof(IvGoodsReceiptListRow.TrxDate), StringComparison.OrdinalIgnoreCase))
        {
            return "TrxDtTime";
        }
    ...
    private static IvGoodsReceiptListQuery Clone(IvGoodsReceiptListQuery source) =>
        new()
        {
