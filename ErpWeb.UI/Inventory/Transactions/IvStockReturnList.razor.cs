...
public partial class IvStockReturnList : PageBase, IDisposable
{
    ...
    private readonly List<IvStockReturnListRow> _selectedRows = [];
    ...
    protected List<IvStockReturnListRow> CompactRows { get; set; } = [];
    ...
    protected IvStockReturnGridDataSource DataSource { get; private set; } = default!;
    ...
    {
        DataSource = new IvStockReturnGridDataSource(SearchPageAsync);

        CanAdd = await AccessRights.CanAsync(MenuCodes.InventoryStockReturn, PermissionCodes.Add);
        CanEdit = await AccessRights.CanAsync(MenuCodes.InventoryStockReturn, PermissionCodes.Edit);
        CanDelete = await AccessRights.CanAsync(MenuCodes.InventoryStockReturn, PermissionCodes.Delete);
        CanPost = await AccessRights.CanAsync(MenuCodes.InventoryStockReturn, PermissionCodes.Post);
        CanRollback = await AccessRights.CanAsync(MenuCodes.InventoryStockReturn, PermissionCodes.Rollback);
        ...
        ActionButtons =
        [
            new() { Text = "VIEW", IConClass = "fa-regular fa-eye", Style = "primary", ToolTip = "View stock return" },
            new() { Text = "EDIT", IConClass = "far fa-edit", Style = "primary", ToolTip = "Edit stock return", Enabled = CanEdit }
        ];
    ...
    protected void OnSelectionsEvent(List<IvStockReturnListRow> list)
    {
        _selectedRows.Clear();
        _selectedRows.AddRange(list);
    }

    protected async Task OnButtonClick(SelectedButtonInfo<IvStockReturnListRow> info)
    {
        ...
        switch (mode)
        {
            case "NEW":
                ...
                Navigation.NavigateTo("/inventory/stock-return/new");
                break;
            case "DELETE":
    ...
    protected Task OnActionClick(SelectedButtonInfo<IvStockReturnListRow> info)
    {
        ...
        switch (mode)
        {
            ...
            case "EDIT":
                ...
                {
                    StatusMessage = "Only NEW stock returns can be edited.";
                    NavigateView(info.SelectedRow.BatchNo);
                    break;
                }

                Navigation.NavigateTo($"/inventory/stock-return/edit/{info.SelectedRow.BatchNo}");
                break;
        }
    ...
    protected void NavigateView(int batchNo) =>
        Navigation.NavigateTo($"/inventory/stock-return/view/{batchNo}");
    ...
    protected async Task ConfirmActionAsync()
    {
        ...
        try
        {
            ...
            IvStockReturnOperationResult result = ConfirmAction switch
            {
                "POST" => await StockReturn.PostAsync(batchNos),
                "ROLLBACK" => await StockReturn.RollbackAsync(batchNos),
                _ => await StockReturn.DeleteAsync(batchNos)
            };
            ...
            if (result.Succeeded)
            {
                ...
                {
                    "POST" => $"Posted {result.SucceededCount} stock return(s).",
                    "ROLLBACK" => $"Rolled back {result.SucceededCount} stock return(s).",
                    _ => "Stock return(s) deleted."
                };
                ConfirmVisible = false;
                _selectedRows.Clear();
                await ReloadGridAsync();
            ...
            {
                ErrorMessage = result.ErrorMessage ?? $"Unable to {ConfirmAction.ToLowerInvariant()} stock return(s).";
                ConfirmVisible = false;
    ...
    private Task BeginDeleteAsync()
    {
        ...
        if (notNew.Count > 0)
        {
            ErrorMessage = $"Only NEW stock returns can be deleted. Non-NEW: {string.Join(", ", notNew)}";
            return Task.CompletedTask;
        }

        ConfirmAction = "DELETE";
        ConfirmMessage = $"Permanently delete {_selectedRows.Count} selected stock return(s)?";
        ConfirmVisible = true;
        return Task.CompletedTask;
    }
    ...
    private Task BeginPostAsync()
    {
        ...
        {
            ErrorMessage = $"Select at most {IvPostingLimits.MaxPostSelection} stock returns to post.";
            return Task.CompletedTask;
        }
        ...
        if (notNew.Count > 0)
        {
            ErrorMessage = $"Only NEW stock returns can be posted. Non-NEW: {string.Join(", ", notNew)}";
            return Task.CompletedTask;
        }

        ConfirmAction = "POST";
        ConfirmMessage = $"Post {_selectedRows.Count} selected stock return(s) to stock?";
        ConfirmVisible = true;
        return Task.CompletedTask;
    }
    ...
    private Task BeginRollbackAsync()
    {
        ...
        if (_selectedRows.Count > IvPostingLimits.MaxPostSelection)
        {
            ErrorMessage = $"Select at most {IvPostingLimits.MaxPostSelection} stock returns to roll back.";
            return Task.CompletedTask;
        }
        ...
        {
            ErrorMessage = $"Only POSTED stock returns can be rolled back. Non-POSTED: {string.Join(", ", notPosted)}";
            return Task.CompletedTask;
        }

        ConfirmAction = "ROLLBACK";
        ConfirmMessage = $"Roll back {_selectedRows.Count} selected stock return(s)? Stock will decrease by the posted quantities.";
        ConfirmVisible = true;
        return Task.CompletedTask;
    }
    ...
    {
        DataSource.UpdateFilters(new IvStockReturnListQuery
        {
    ...
    private async Task RefreshCompactPreviewAsync()
    {
        ...
        query.Skip = 0;
        query.Take = 50;
        var result = await StockReturn.SearchAsync(query);
    ...
    private async Task<(IReadOnlyList<IvStockReturnListRow> Rows, int TotalCount)> SearchPageAsync(
        IvStockReturnListQuery query,
        CancellationToken cancellationToken)
    {
        var result = await StockReturn.SearchAsync(query, cancellationToken);
        if (!result.Succeeded || result.ListPage is null)
        {
            await InvokeAsync(() =>
            {
                ErrorMessage = result.ErrorMessage ?? "Unable to load stock returns.";
                TotalCount = 0;
            });
            return ([], 0);
        }
...
/// <summary>
/// Server-side paging source for Stock Return via <see cref="IIvStockReturnService.SearchAsync"/>.
/// </summary>
public sealed class IvStockReturnGridDataSource : GridCustomDataSource
{
    private readonly Func<IvStockReturnListQuery, CancellationToken, Task<(IReadOnlyList<IvStockReturnListRow> Rows, int TotalCount)>> _loader;
    private IvStockReturnListQuery _filters = new();

    public IvStockReturnGridDataSource(
        Func<IvStockReturnListQuery, CancellationToken, Task<(IReadOnlyList<IvStockReturnListRow> Rows, int TotalCount)>> loader)
    {
        _loader = loader;
    }

    public IvStockReturnListQuery CurrentQuery => Clone(_filters);

    public void UpdateFilters(IvStockReturnListQuery query) =>
    ...
    {
        if (string.Equals(fieldName, nameof(IvStockReturnListRow.TrxDate), StringComparison.OrdinalIgnoreCase))
        {
            return "TrxDtTime";
        }
    ...
    private static IvStockReturnListQuery Clone(IvStockReturnListQuery source) =>
        new()
        {
