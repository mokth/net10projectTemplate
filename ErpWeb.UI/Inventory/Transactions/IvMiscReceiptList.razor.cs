...
public partial class IvMiscReceiptList : PageBase, IDisposable
{
    ...
    private readonly List<IvMiscReceiptListRow> _selectedRows = [];
    ...
    protected List<IvMiscReceiptListRow> CompactRows { get; set; } = [];
    ...
    protected IvMiscReceiptGridDataSource DataSource { get; private set; } = default!;
    ...
    {
        DataSource = new IvMiscReceiptGridDataSource(SearchPageAsync);

        CanAdd = await AccessRights.CanAsync(MenuCodes.InventoryMiscReceipt, PermissionCodes.Add);
        CanEdit = await AccessRights.CanAsync(MenuCodes.InventoryMiscReceipt, PermissionCodes.Edit);
        CanDelete = await AccessRights.CanAsync(MenuCodes.InventoryMiscReceipt, PermissionCodes.Delete);
        CanPost = await AccessRights.CanAsync(MenuCodes.InventoryMiscReceipt, PermissionCodes.Post);
        CanRollback = await AccessRights.CanAsync(MenuCodes.InventoryMiscReceipt, PermissionCodes.Rollback);
    ...
    protected void OnSelectionsEvent(List<IvMiscReceiptListRow> list)
    {
        _selectedRows.Clear();
        _selectedRows.AddRange(list);
    }

    protected async Task OnButtonClick(SelectedButtonInfo<IvMiscReceiptListRow> info)
    {
        ...
        switch (mode)
        {
            case "NEW":
                ...
                Navigation.NavigateTo("/inventory/misc-receipt/new");
                break;
            case "DELETE":
    ...
    protected Task OnActionClick(SelectedButtonInfo<IvMiscReceiptListRow> info)
    {
        ...
        switch (mode)
        {
            ...
            case "EDIT":
                ...
                Navigation.NavigateTo($"/inventory/misc-receipt/edit/{info.SelectedRow.BatchNo}");
                break;
        }
    ...
    protected void NavigateView(int batchNo) =>
        Navigation.NavigateTo($"/inventory/misc-receipt/view/{batchNo}");
    ...
    protected async Task ConfirmActionAsync()
    {
        ...
        try
        {
            ...
            IvMiscReceiptOperationResult result = ConfirmAction switch
            {
                "POST" => await MiscReceipt.PostAsync(batchNos),
                "ROLLBACK" => await MiscReceipt.RollbackAsync(batchNos),
                _ => await MiscReceipt.DeleteAsync(batchNos)
            };
    ...
    {
        DataSource.UpdateFilters(new IvMiscReceiptListQuery
        {
    ...
    private async Task RefreshCompactPreviewAsync()
    {
        ...
        query.Skip = 0;
        query.Take = 50;
        var result = await MiscReceipt.SearchAsync(query);
    ...
    private async Task<(IReadOnlyList<IvMiscReceiptListRow> Rows, int TotalCount)> SearchPageAsync(
        IvMiscReceiptListQuery query,
        CancellationToken cancellationToken)
    {
        var result = await MiscReceipt.SearchAsync(query, cancellationToken);
...
/// <summary>
/// Server-side paging source for Miscellaneous Receipt via <see cref="IIvMiscReceiptService.SearchAsync"/>.
/// </summary>
public sealed class IvMiscReceiptGridDataSource : GridCustomDataSource
{
    private readonly Func<IvMiscReceiptListQuery, CancellationToken, Task<(IReadOnlyList<IvMiscReceiptListRow> Rows, int TotalCount)>> _loader;
    private IvMiscReceiptListQuery _filters = new();

    public IvMiscReceiptGridDataSource(
        Func<IvMiscReceiptListQuery, CancellationToken, Task<(IReadOnlyList<IvMiscReceiptListRow> Rows, int TotalCount)>> loader)
    {
        _loader = loader;
    }

    public IvMiscReceiptListQuery CurrentQuery => Clone(_filters);

    public void UpdateFilters(IvMiscReceiptListQuery query) =>
    ...
    {
        if (string.Equals(fieldName, nameof(IvMiscReceiptListRow.TrxDate), StringComparison.OrdinalIgnoreCase))
        {
            return "TrxDtTime";
        }
    ...
    private static IvMiscReceiptListQuery Clone(IvMiscReceiptListQuery source) =>
        new()
        {
