...
public partial class IvMiscIssueList : PageBase, IDisposable
{
    ...
    private readonly List<IvMiscIssueListRow> _selectedRows = [];
    ...
    protected List<IvMiscIssueListRow> CompactRows { get; set; } = [];
    ...
    protected IvMiscIssueGridDataSource DataSource { get; private set; } = default!;
    ...
    [
        ...
        new() { Caption = "Total", FieldName = nameof(IvMiscIssueListRow.TotalAmount), DataType = "decimal", DisplayFormat = "n2", Width = "110px", VisibleIndex = 7 },
        new() { Caption = "Created", FieldName = nameof(IvMiscIssueListRow.CreatedDate), DataType = "date", DisplayFormat = "dd/MM/yyyy HH:mm", Width = "140px", VisibleIndex = 8 },
        new() { Caption = "Created by", FieldName = nameof(IvMiscIssueListRow.CreatedBy), Visible = false, VisibleIndex = 9 }
    ];
    ...
    {
        DataSource = new IvMiscIssueGridDataSource(SearchPageAsync);

        CanAdd = await AccessRights.CanAsync(MenuCodes.InventoryMiscIssue, PermissionCodes.Add);
        CanEdit = await AccessRights.CanAsync(MenuCodes.InventoryMiscIssue, PermissionCodes.Edit);
        CanDelete = await AccessRights.CanAsync(MenuCodes.InventoryMiscIssue, PermissionCodes.Delete);
        CanPost = await AccessRights.CanAsync(MenuCodes.InventoryMiscIssue, PermissionCodes.Post);
        CanRollback = await AccessRights.CanAsync(MenuCodes.InventoryMiscIssue, PermissionCodes.Rollback);
        CanCancel = await AccessRights.CanAsync(MenuCodes.InventoryMiscIssue, PermissionCodes.Cancel);
    ...
    protected void OnSelectionsEvent(List<IvMiscIssueListRow> list)
    {
        _selectedRows.Clear();
        _selectedRows.AddRange(list);
    }

    protected async Task OnButtonClick(SelectedButtonInfo<IvMiscIssueListRow> info)
    {
        ...
        switch (mode)
        {
            case "NEW":
                ...
                Navigation.NavigateTo("/inventory/misc-issue/new");
                break;
            case "DELETE":
    ...
    protected Task OnActionClick(SelectedButtonInfo<IvMiscIssueListRow> info)
    {
        ...
        switch (mode)
        {
            ...
            case "EDIT":
                ...
                Navigation.NavigateTo($"/inventory/misc-issue/edit/{info.SelectedRow.BatchNo}");
                break;
        }
    ...
    protected void NavigateView(int batchNo) =>
        Navigation.NavigateTo($"/inventory/misc-issue/view/{batchNo}");
    ...
    protected async Task ConfirmActionAsync()
    {
        ...
        try
        {
            ...
            IvMiscIssueOperationResult result = ConfirmAction switch
            {
                "POST" => await MiscIssue.PostAsync(batchNos),
                "ROLLBACK" => await MiscIssue.RollbackAsync(batchNos),
                "CANCEL" => await MiscIssue.CancelAsync(batchNos),
                _ => await MiscIssue.DeleteAsync(batchNos)
            };
    ...
    {
        DataSource.UpdateFilters(new IvMiscIssueListQuery
        {
    ...
    private async Task RefreshCompactPreviewAsync()
    {
        ...
        query.Skip = 0;
        query.Take = 50;
        var result = await MiscIssue.SearchAsync(query);
    ...
    private async Task<(IReadOnlyList<IvMiscIssueListRow> Rows, int TotalCount)> SearchPageAsync(
        IvMiscIssueListQuery query,
        CancellationToken cancellationToken)
    {
        var result = await MiscIssue.SearchAsync(query, cancellationToken);
...
/// <summary>
/// Server-side paging source for Miscellaneous Issue via <see cref="IIvMiscIssueService.SearchAsync"/>.
/// </summary>
public sealed class IvMiscIssueGridDataSource : GridCustomDataSource
{
    private readonly Func<IvMiscIssueListQuery, CancellationToken, Task<(IReadOnlyList<IvMiscIssueListRow> Rows, int TotalCount)>> _loader;
    private IvMiscIssueListQuery _filters = new();

    public IvMiscIssueGridDataSource(
        Func<IvMiscIssueListQuery, CancellationToken, Task<(IReadOnlyList<IvMiscIssueListRow> Rows, int TotalCount)>> loader)
    {
        _loader = loader;
    }

    public IvMiscIssueListQuery CurrentQuery => Clone(_filters);

    public void UpdateFilters(IvMiscIssueListQuery query) =>
    ...
    {
        if (string.Equals(fieldName, nameof(IvMiscIssueListRow.TrxDate), StringComparison.OrdinalIgnoreCase))
        {
            return "TrxDtTime";
        }
    ...
    private static IvMiscIssueListQuery Clone(IvMiscIssueListQuery source) =>
        new()
        {
