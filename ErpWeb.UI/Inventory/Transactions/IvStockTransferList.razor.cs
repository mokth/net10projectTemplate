...
public partial class IvStockTransferList : PageBase, IDisposable
{
    ...
    private readonly List<IvStockTransferListRow> _selectedRows = [];
    ...
    protected List<IvStockTransferListRow> CompactRows { get; set; } = [];
    ...
    protected IvStockTransferGridDataSource DataSource { get; private set; } = default!;
    ...
    [
        new() { Caption = "Batch No", FieldName = nameof(IvStockTransferListRow.BatchNo), Width = "100px", SortIndex = 0, VisibleIndex = 1 },
        new() { Caption = "Date", FieldName = nameof(IvStockTransferListRow.TrxDate), DataType = "date", DisplayFormat = "dd/MM/yyyy", Width = "110px", VisibleIndex = 2 },
        new() { Caption = "Status", FieldName = nameof(IvStockTransferListRow.BatchStatus), Width = "100px", VisibleIndex = 3 },
        ...
        new() { Caption = "Created", FieldName = nameof(IvStockTransferListRow.CreatedDate), DataType = "date", DisplayFormat = "dd/MM/yyyy HH:mm", Width = "140px", VisibleIndex = 8 },
        new() { Caption = "Created by", FieldName = nameof(IvStockTransferListRow.CreatedBy), Visible = false, VisibleIndex = 9 }
    ];
    ...
    {
        DataSource = new IvStockTransferGridDataSource(SearchPageAsync);

        CanAdd = await AccessRights.CanAsync(MenuCodes.InventoryStockTransfer, PermissionCodes.Add);
        CanEdit = await AccessRights.CanAsync(MenuCodes.InventoryStockTransfer, PermissionCodes.Edit);
        CanDelete = await AccessRights.CanAsync(MenuCodes.InventoryStockTransfer, PermissionCodes.Delete);
        CanPost = await AccessRights.CanAsync(MenuCodes.InventoryStockTransfer, PermissionCodes.Post);
        CanRollback = await AccessRights.CanAsync(MenuCodes.InventoryStockTransfer, PermissionCodes.Rollback);
        CanCancel = await AccessRights.CanAsync(MenuCodes.InventoryStockTransfer, PermissionCodes.Cancel);
    ...
    protected void OnSelectionsEvent(List<IvStockTransferListRow> list)
    {
        _selectedRows.Clear();
        _selectedRows.AddRange(list);
    }

    protected async Task OnButtonClick(SelectedButtonInfo<IvStockTransferListRow> info)
    {
        ...
        switch (mode)
        {
            case "NEW":
                ...
                Navigation.NavigateTo("/inventory/stock-transfer/new");
                break;
            case "DELETE":
    ...
    protected Task OnActionClick(SelectedButtonInfo<IvStockTransferListRow> info)
    {
        ...
        switch (mode)
        {
            ...
            case "EDIT":
                ...
                Navigation.NavigateTo($"/inventory/stock-transfer/edit/{info.SelectedRow.BatchNo}");
                break;
        }
    ...
    protected void NavigateView(int batchNo) =>
        Navigation.NavigateTo($"/inventory/stock-transfer/view/{batchNo}");
    ...
    protected async Task ConfirmActionAsync()
    {
        ...
        try
        {
            ...
            IvStockTransferOperationResult result = ConfirmAction switch
            {
                "POST" => await StockTransferService.PostAsync(batchNos),
                "ROLLBACK" => await StockTransferService.RollbackAsync(batchNos),
                "CANCEL" => await StockTransferService.CancelAsync(batchNos),
                _ => await StockTransferService.DeleteAsync(batchNos)
            };
    ...
    {
        DataSource.UpdateFilters(new IvStockTransferListQuery
        {
    ...
    private async Task RefreshCompactPreviewAsync()
    {
        ...
        query.Skip = 0;
        query.Take = 50;
        var result = await StockTransferService.SearchAsync(query);
    ...
    private async Task<(IReadOnlyList<IvStockTransferListRow> Rows, int TotalCount)> SearchPageAsync(
        IvStockTransferListQuery query,
        CancellationToken cancellationToken)
    {
        var result = await StockTransferService.SearchAsync(query, cancellationToken);
...
/// <summary>
/// Server-side paging source for Stock transfer via <see cref="IIvStockTransferService.SearchAsync"/>.
/// </summary>
public sealed class IvStockTransferGridDataSource : GridCustomDataSource
{
    private readonly Func<IvStockTransferListQuery, CancellationToken, Task<(IReadOnlyList<IvStockTransferListRow> Rows, int TotalCount)>> _loader;
    private IvStockTransferListQuery _filters = new();

    public IvStockTransferGridDataSource(
        Func<IvStockTransferListQuery, CancellationToken, Task<(IReadOnlyList<IvStockTransferListRow> Rows, int TotalCount)>> loader)
    {
        _loader = loader;
    }

    public IvStockTransferListQuery CurrentQuery => Clone(_filters);

    public void UpdateFilters(IvStockTransferListQuery query) =>
    ...
    {
        if (string.Equals(fieldName, nameof(IvStockTransferListRow.TrxDate), StringComparison.OrdinalIgnoreCase))
        {
            return "TrxDtTime";
        }
    ...
    private static IvStockTransferListQuery Clone(IvStockTransferListQuery source) =>
        new()
        {
