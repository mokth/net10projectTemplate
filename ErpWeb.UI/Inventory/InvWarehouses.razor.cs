...
public partial class InvWarehouses : PageBase
{
    [Inject] private IWarehouseService Service { get; set; } = default!;
    ...
    protected Warehouse selectedRow { get; set; } = new() { IsActive = true };
    protected IEnumerable<Warehouse>? Data;
    private readonly List<Warehouse> _selected = [];

    public List<GridColumnData> Columns() =>
    [
        new() { Caption = "Code", FieldName = nameof(Warehouse.WarehouseCode), DataType = "string", SortIndex = 0, SortOrder = GridColumnSortOrder.Ascending, VisibleIndex = 1, Width = "120px" },
        new() { Caption = "Name", FieldName = nameof(Warehouse.WarehouseName), DataType = "string", VisibleIndex = 2 },
        new() { Caption = "Active", FieldName = nameof(Warehouse.IsActive), DataType = "bool", VisibleIndex = 3, Width = "80px" }
    ];
    ...
    {
        _selected.Clear();
        _selected.AddRange(rows.OfType<Warehouse>());
    }

    public async Task OnActionClick(SelectedButtonInfo<Warehouse> info)
    {
        if (info.SelectedRow is null) return;
        var view = info.SelectedButton.Text.Equals("View", StringComparison.OrdinalIgnoreCase);
        if (!await AccessRights.CanAsync(MenuCodes.InvWarehouses, view ? PermissionCodes.Access : PermissionCodes.Edit))
    ...
    public async Task OnButtonClick(SelectedButtonInfo<Warehouse> info)
    {
        if (info.SelectedButton.Text.Equals("New", StringComparison.OrdinalIgnoreCase))
        {
            if (!await AccessRights.CanAsync(MenuCodes.InvWarehouses, PermissionCodes.Add)) { StatusMessage = "Access Denied!!"; return; }
            selectedRow = new Warehouse { IsActive = true };
        ...
        {
            if (!await AccessRights.CanAsync(MenuCodes.InvWarehouses, PermissionCodes.Delete)) { StatusMessage = "Access Denied!!"; return; }
            if (_selected.Count == 0) { StatusMessage = "No Record Selected!"; return; }
            if (!await Js.InvokeAsync<bool>("confirm", $"Delete {_selected.Count} warehouse(s)?")) return;
    ...
    protected async Task HandleValidSubmit()
    {
        ...
        try
        {
            ...
            PopupVisible = false;
            StatusMessage = IsEditMode ? "Updated." : "Created with MAIN location.";
            await LoadAsync();
        }
        finally { IsSubmitting = false; }
    }

    private static Warehouse Clone(Warehouse s) => new()
    {
        Id = s.Id, CompanyId = s.CompanyId, BranchId = s.BranchId,
        WarehouseCode = s.WarehouseCode, WarehouseName = s.WarehouseName,
