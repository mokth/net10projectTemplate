...
/// <summary>
/// Shared helpers for sales reference master list + popup pages with row-version keys.
/// </summary>
public abstract class SaRefListPageBase<TRow> : PageBase
    where TRow : class
{
    ...
    protected static string FormatResultMessage<T>(IvMasterOperationResult<T> result) =>
        SaRefListMessages.FormatResultMessage(result);

    protected static string FormatDeleteBlocked(DeleteCheckResult check) =>
        SaRefListMessages.FormatDeleteBlocked(check);
    ...
    protected static SaCompanyMasterKeyToken ToSaKey(IvMasterKeyToken token) =>
        new()
        {
            Code = token.Code,
    ...
    protected static IReadOnlyList<SaCompanyMasterKeyToken> ToSaKeys(IReadOnlyList<IvMasterKeyToken> items) =>
        items.Select(ToSaKey).ToList();
}

internal static class SaRefListMessages
{
    public static string FormatResultMessage<T>(IvMasterOperationResult<T> result)
    {
    ...
    public static string FormatDeleteBlocked(DeleteCheckResult check)
    {
