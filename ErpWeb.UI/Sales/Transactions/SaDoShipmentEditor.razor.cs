...
public partial class SaDoShipmentEditor : ComponentBase
{
    [Parameter] public string DoNo { get; set; } = string.Empty;
    [Parameter] public int SoLineNo { get; set; }
    [Parameter] public byte[]? RowVersion { get; set; }
    [Parameter] public bool Enabled { get; set; } = true;
    [Parameter] public EventCallback OnCancelled { get; set; }
    [Parameter] public EventCallback<SaDoDocument> OnApplied { get; set; }
    [Parameter] public EventCallback<(string Message, SaDoDocument? Document)> OnApplyFailed { get; set; }

    [Inject] private ISaDoService DoService { get; set; } = default!;
    ...
    private async Task LoadSnapshotAsync()
    {
        ErrorMessage = null;
        _rows.Clear();
        var result = await DoService.GetShipmentEditAsync(DoNo, SoLineNo);
    ...
    private async Task OnApplyAsync()
    {
        ...
        try
        {
            var lots = _rows
                ...
                .Select(x => new SaDoShipmentLotRequest
                {
            ...
            var result = await DoService.ReplaceShipmentLineAsync(DoNo, SoLineNo, lots, RowVersion);
