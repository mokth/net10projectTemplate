export async function uploadPoAttachment(docId, file, antiforgeryToken) {
    const id = encodeURIComponent((docId ?? "").trim());
    const formData = new FormData();
    formData.append("file", file);

    try {
        const response = await fetch(`/purchase/orders/attachments/${id}`, {
            method: "POST",
            body: formData,
            credentials: "include",
...
export async function uploadPoAttachmentFromInput(input, docId, antiforgeryToken) {
    const file = input?.files?.[0];
    if (!file) {
        return { ok: false, status: 0, message: "Choose a file first." };
    }

    const result = await uploadPoAttachment(docId, file, antiforgeryToken);
    if (input) {
        input.value = "";
    }
    return result;
